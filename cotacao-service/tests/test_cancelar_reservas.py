...
from app.escrita import cancelar_reservas, reservar_roteiro
from app.leitura import consultar_extrato
from app.models import CANCELADA, Orcamento, Reserva


def test_cancelar_devolve_vagas_e_valor(session, catalogo, novo_pedido, hoje):
    ...
    assert {r.status for r in session.exec(select(Reserva)).all()} == {CANCELADA}


def test_cancelar_duas_vezes_nao_devolve_em_dobro(session, catalogo, novo_pedido, hoje):
    reservar_roteiro(session, novo_pedido(teto=3000.0), hoje=hoje)
...
def test_da_para_reservar_novamente_depois_de_cancelar(session, catalogo, novo_pedido, hoje):
    reservar_roteiro(session, novo_pedido(teto=3000.0), hoje=hoje)
    cancelar_reservas(session, roteiro_id=1)

    resultado = reservar_roteiro(session, novo_pedido(teto=4000.0), hoje=hoje)
...
def test_extrato_apos_cancelamento_mostra_saldo_integral(session, catalogo, novo_pedido, hoje):
    reservar_roteiro(session, novo_pedido(teto=3000.0), hoje=hoje)
    cancelar_reservas(session, roteiro_id=1)
